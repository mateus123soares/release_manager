...
export class ProjectController {
  ...
  const client = await Request.server.pg.connect();
    try {
      const rows = await this.projectService.postCreateProject(client, { "name": Request.body.name, "description": Request.body.description, "gitlab_project_id": Request.body.gitlab_project_id});
      return reply.status(200).send(rows);
    } finally {
      client.release();
    }
