...
export class ReleaseNoteService {
  ...
  async getLastTag(projectID: number) {
    try {
      ...
      this.logger.info(`Info: buscando a ultima tag do projeto ${projectID}`);
      const tag = await axios.request(axiosConfig);
      return tag.data[1].commit.created_at;
    } catch (error) {
      this.logger.warn(
        `Aviso: Nenhuma tag encontrada. Analisando todos os commits. ${error}`,
      );
      return null;
    }
  }
  ...
  async getCommitsSinceLastTag(
    ...
    try {
      this.logger.info(`Info: buscando commits do projeto ${projectID}`);
      const commitsResponse =
        await axios.request<GitLabCommit[]>(commitsConfig);
      return commitsResponse.data;
    } catch (error: unknown) {
      if (error instanceof Error) {
        this.logger.error(
          'Erro: A última tag não existe ou o repositório está vazio. Verifique se há commits.',
        );
        throw error;
      }
    }
  ...
  generateReleaseNote(
    parsedCommits: GitLabCommitParsed[],
    filename: string,
    outputDir: string,
  ...
  - **${commit.title}** (${commit.id}) por ${commit.author_email} em ${commit.created_at}
    ...
    this.logger.info(`Info: Notas de lançamento salvas em: ${filePath}`);
  }
  ...
  async generateReleaseNotes(metadata: ReleaseNote) {
    ...
    this.generateReleaseNote(
      parsedCommits,
      `release_notes_${metadata.project}.md`,
      path,
      metadata,
    );
  }
